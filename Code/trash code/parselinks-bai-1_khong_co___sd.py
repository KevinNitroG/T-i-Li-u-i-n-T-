...
def parser(filename, type, chapter=None):
	...
	for i in data['log']['entries']:
		try:
			...
			if ('.mp4' in content) and ('m3u8' in content): 
				links.append(content)
		except Exception:
			pass

	links = [f"{i.replace('__sd', '')[:i.find('?nimblesessionid')-0]}" for i in links]
...
res = parser('casper_mono.har', 3, 'Video Sửa chữa Bo tủ lạnh Panasonic Mono')
print(res)
